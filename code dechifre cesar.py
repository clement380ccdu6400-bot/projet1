...
def conv_cesar(mot_sepa,mot_conteur):
    mot_conv = []
    cont=0
    for x in mot_sepa:
        if x in alpha:
            x=alpha[(mot_conteur[cont%len(mot_conteur)]-2)%26]
            cont+=1
            mot_conv.append(x)
        else:
            cont=cont+1
...
mot_conv = conv_cesar(mot_sepa,mot_conteur)
texte_dechiffre = texte_final(mot_conv)

print("Texte chiffré :", texte_dechiffre)
